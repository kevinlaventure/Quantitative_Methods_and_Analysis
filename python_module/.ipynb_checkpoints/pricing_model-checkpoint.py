    ...
    def compute_option(S, K, T, r, sigma, option_type, compute_greeks):
        ...
        if compute_greeks:
            if T > 0:
                ...
                return {"price": price, "delta": delta, "gamma": gamma, "gamma_cash": (gamma*(S**2))/100, "vega": vega, "theta": theta, "vanna": vanna, "volga": volga}
            else:
                return {"price": price, "delta": np.nan, "gamma": np.nan, "gamma_cash": (gamma*(S**2))/100, "vega": np.nan, "theta": np.nan, "vanna": np.nan, "volga": np.nan}
        else:
            return price
...
class SABRModel:
    ...
    @staticmethod
    def compute_option(F, K, T, alpha, beta, rho, nu, r, option_type, slide_list=[], slide_type='spot_vol', slide_compute='delta_hedged_pnl'):
        ...
        for slide in slide_list:
            if slide_type == 'spot_vol':
                ...
                results_bumped = BlackScholesModel.compute_option(S0_bumped, K, T, r, IV_bumped, option_type, True)
            elif slide_type == 'spot_only':
                IV = SABRModel.compute_sigma(F, K, T, alpha, beta, rho, nu)
                F_bumped = F * (1 + slide)
                S0_bumped = F_bumped * np.exp(-r * T)
                results_bumped = BlackScholesModel.compute_option(S0_bumped, K, T, r, IV, option_type, True)
            if slide_compute == 'delta_hedged_pnl':
                option_pnl = results_bumped['price'] - pricing_results['price']
                ...
                delta_hedge_pnl = S0*pricing_results['delta']*slide*-1
                pricing_results[slide] = delta_hedge_pnl
            else:
                pricing_results[slide] = results_bumped[slide_compute]
