...
DEFAULT_TARGET_COLUMNS: Sequence[str] = (
    "costof_gamma",
    "costof_vanna",
    "costof_volga",
    "costof_other",
...
def _resolve_target_columns(
    ...
    if target_columns is None:
        ...
        candidate_columns = [
            col for col in DEFAULT_TARGET_COLUMNS if col in df.columns and col in index_like
        ]
...
def find_min_abs_weights(
    ...
    *,
    ...
    target_columns:
        Iterable of exposure column names to match. Defaults to the entries in
        ``DEFAULT_TARGET_COLUMNS`` that are present in both ``df`` and
        ``index_cog``.
    combination_size:
